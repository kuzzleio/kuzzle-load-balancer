...
module.exports = function pluginsInstall (applicationName) {
  ...
  if (!fs.existsSync(getNodeModuleFolder())) {
    fs.mkdirSync(getNodeModuleFolder());
  }
...
function getPathPlugin (pluginConfig, pluginName) {
  ...
  return path.join(getNodeModuleFolder(), pluginName);
}

function getNodeModuleFolder () {
