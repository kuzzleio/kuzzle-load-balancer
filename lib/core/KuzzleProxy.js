 ...
 * @constructor
 */
function KuzzleProxy () {
  /** @type {Object} */
  this.config = this.getRCConfig();

  if (['failover', 'round-robin'].indexOf(this.config.backendMode) === -1) {
    throw new Error(`Backend mode option must be either set to "failover" or "round-robin"; "${this.config.backendMode}" given`);
  }
 ...
 * @returns {Object}
 */
KuzzleProxy.prototype.getRCConfig = function () {
...
KuzzleProxy.prototype.initBroker = function () {
  ...
  this.broker.init(this.config.backendMode, this.context, this.config.backendOptions, this.config.backendTimeout);
};
