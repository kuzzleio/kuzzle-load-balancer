...
class KuzzleProxy {
    ...
    pluginList.forEach((pluginDirName) => {
      ...
      try {
        if (fs.existsSync(path.resolve(pluginPath, 'package.json'))) {
          pluginDefinition = this.loadPluginFromPackageJson(path.resolve(pluginsDir, pluginDirName));
        } else {
          pluginDefinition = this.loadPluginFromDirectory(path.resolve(pluginsDir, pluginDirName));
        }
      } catch (e) {
  ...
  loadPluginFromPackageJson (pluginPath) {
    ...
    return {
      ...
      config: this.config[packageJson.name] || {},
      path: pluginPath
    };
  }
  ...
  initPlugin (pluginDefinition) {
    ...
    try {
      ...
      this.pluginStore.add(pluginDefinition.object);
    } catch (error) {
