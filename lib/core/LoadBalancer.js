...
LoadBalancer.prototype.initPlugins = function () {
  ...
  Object.keys(pluginsConfiguration).forEach((pluginName) => {
    ...
    if (pluginsConfiguration[pluginName].activated) {
...
LoadBalancer.prototype.readPluginsConfiguration = function () {
  ...
  if (Object.keys(plugins).length === 0) {
    throw new Error('No plugin configuration provided. We shutdown.');
  }
  ...
  Object.keys(plugins).forEach((pluginName) => {
    ...
    pluginsConfiguration[pluginName] = pluginConfiguration;
  });
  ...
  if (activatedPlugins.length === 0) {
    throw new Error('No plugin has been activated in configuration. We shutdown.');
  }
