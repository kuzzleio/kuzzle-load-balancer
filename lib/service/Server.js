...
Server.prototype.onMessage = function (data) {
  ...
  switch (dataObject.room) {
    case 'response':
      if (this.pendingRequestStore.existsByRequestId(dataObject.requestId)) {
        responseObjectWrapper = new this.context.ResponseObject(dataObject.data);
      ...
      break;
    case 'notify':
      clientProtocol = this.context.clientConnectionStore.getByConnectionId(dataObject.connection.id).type;
      this.context.pluginStore.getByProtocol(clientProtocol).notify(dataObject.message);
      break;
    case 'broadcast':
      clientProtocol = this.context.clientConnectionStore.getByConnectionId(dataObject.connection.id).type;
      dataObject.message.id = dataObject.connection.id;
      this.context.pluginStore.getByProtocol(clientProtocol).notify(dataObject.message);
      break;
    case 'httpPortInitialization':
      ...
      break;
    default:
      console.error(`plugin-websocket-demultiplexer: Unknown message type ${dataObject.room}`);
      break;
  }
};
