...
Server.prototype.onMessage = function (data) {
  ...
  switch (message.room) {
    ...
    case 'notify':
      ...
      break;
    case 'broadcast':
      _.forEach(this.context.pluginStore.plugins, (plugin) => {
        plugin.broadcast(message.data);
      });
      break;
    case 'httpPortInitialization':
      this.httpPort = message.httpPort;
      break;
    default:
      console.error(`plugin-websocket-demultiplexer: Unknown message type ${message.room}`);
      break;
  }
};
