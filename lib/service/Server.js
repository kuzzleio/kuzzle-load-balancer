...
Server.prototype.onMessage = function (data) {
  ...
  if (!dataObject.requestId && dataObject.httpPort) {
    this.httpPort = dataObject.httpPort;
  }
  else if (dataObject.requestId && this.pendingRequestStore.existsByRequestId(dataObject.requestId)) {
    responseObjectWrapper = new this.context.ResponseObject(dataObject);
