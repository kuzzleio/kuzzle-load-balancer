...
class HttpProxy {
  ...
  init(proxy) {
    ...
    this.server = http.createServer((request, response) => {
      ...
      debug('[%s] receiving HTTP request:\n%O', connection.id, payload);
      ...
      if (request.headers['content-length'] > this.maxRequestSize) {
        request
          .removeAllListeners()
          .resume();
        return replyWithError(this.proxy, connection.id, payload, response, new SizeLimitError('Error: maximum HTTP request size exceeded'));
      }

      if (!request.headers['content-type'] || request.headers['content-type'].startsWith('application/json')) {
        stream = new Writable({
          write(chunk, encoding, callback) {
            debug('[%s] writing chunk:\n%s', connection.id, chunk.toString());
        ...
        stream.on('error', err => {
          ...
          request
            ...
            .resume();
          return replyWithError(this.proxy, connection.id, payload, response, err);
        });
      } else {
        try {
          ...
          request.resume();
          return replyWithError(this.proxy, connection.id, payload, response, new BadRequestError(error.message));
        }
      ...
      request.on('end', () => {
        ...
        stream.end(() => {
          ...
          sendRequest(this.proxy, connection.id, response, payload);
        });
      });
    });
 ...
 */
function sendRequest (proxy, connectionId, response, payload) {
  debug('[%s] sendRequest:\n%O', connectionId, payload);
  if (payload.json) {
    payload.content = JSON.stringify(payload.json);
    delete payload.json;
  }

  proxy.broker.brokerCallback(KuzzleRoom, payload.requestId, connectionId, payload, (error, result) => {
    debug('[%s] brokerCallback:\n%O\n%O', connectionId, error, result);
    if (result) {
      proxy.clientConnectionStore.remove(connectionId);
 ...
 * @param {Object} error
 */
function replyWithError(proxy, connectionId, payload, response, error) {
  const result = {
    raw: true,
    content: JSON.stringify(error)
  };

  debug('[%s] replyWithError:\n%O', connectionId, error);

  proxy.logAccess(connectionId, payload, error, result);
