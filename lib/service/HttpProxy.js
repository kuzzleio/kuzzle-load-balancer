...
const
  debug = require('../kuzzleDebug')('kuzzle-proxy:httpProxy'),
  http = require('http'),
  bytes = require('bytes'),
  uuid = require('uuid'),
  url = require('url'),
...
class HttpProxy {
  ...
  init(proxy) {
    ...
    this.maxEncodingLayers = proxy.config.http.maxEncodingLayers;

    this.proxy = proxy;

    for (const numericParameter of ['maxRequestSize', 'maxEncodingLayers', 'maxFormFileSize']) {
      if (this[numericParameter] === null || isNaN(this[numericParameter])) {
        throw new Error(`Invalid "${numericParameter}" parameter value: expected a numeric value`);
      }
    }
    ...
    this.server = http.createServer((request, response) => {
      ...
      if (request.headers['x-forwarded-for']) {
        request.headers['x-forwarded-for'].split(',').forEach(s => payload.ips.push(s.trim()));
      }

      const connection = new ClientConnection('HTTP/' + request.httpVersion, payload.ips, request.headers);
  ...
  _setDecoders() {
    const
      allowCompression = this.proxy.config.http.allowCompression,
      disabledfn = () => {
        throw new BadRequestError('Compression support is disabled');
      };

    if (typeof allowCompression !== 'boolean') {
      throw new Error('Invalid "allowCompression" parameter value: expected a boolean value');
    }
  ...
  _sendRequest (connectionId, response, payload) {
    ...
    this.proxy.broker.brokerCallback(KuzzleRoom, payload.requestId, connectionId, payload, (error, result) => {
      debug('[%s] brokerCallback:\nerror: %O\nresult: %O', connectionId, error, result);

      if (result) {
        debug('[%s] broker response success callback: %a', connectionId, result);

        this.proxy.clientConnectionStore.remove(connectionId);
        ...
        const data = this._contentToPayload(result, payload.url);
        ...
        this._replyWithError(connectionId, payload, response, error);
      }
    });
  }
  ...
  _replyWithError(connectionId, payload, response, error) {
    ...
    this.proxy.logAccess(connectionId, payload, error, result);
