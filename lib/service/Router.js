...
Router.prototype.newConnection = function (protocol, socketId) {
  ...
  currentConnection = this.context.clientConnectionStore.get(connectionAttempt);
...
Router.prototype.execute = function (requestObject, connection, callback) {
  this.context.broker.brokerCallback(this.context.broker.addEnvelope(requestObject, connection, 'request'), (error, result) => {
    if (error) {
      return callback(null, new this.context.constructors.ResponseObject(requestObject, error));
    }
