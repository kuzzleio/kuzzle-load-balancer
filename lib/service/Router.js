...
Router.prototype.execute = function (requestObject, connection, callback) {
  var
    resolve,
    reject,
    requestPromise = new Promise((rs, rj) => {
      resolve = rs;
      reject = rj;
    });

  this.context.broker.brokerCallback(this.context.broker.addEnvelope(requestObject, connection, 'request'), {resolve, reject, requestPromise});

  return requestPromise
    .then(responseObject => callback(null, responseObject))
    .catch(error => callback(null, new this.context.constructors.ResponseObject(requestObject, error)));
};
