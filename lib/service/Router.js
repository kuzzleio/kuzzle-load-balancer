...
class Router {
   ...
   */
  execute(data, connection, callback) {
    const request = new Request(data, {connection});

    this.proxy.broker.brokerCallback('request', request.id, request.context.connection.id, request.serialize(), (error, result) => {
      if (error) {
