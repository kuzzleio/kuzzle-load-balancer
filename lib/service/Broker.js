...
class Broker {
  ...
  onConnection(backendSocket) {
    const
      req = backendSocket.upgradeReq,
      backend = new Backend(backendSocket, req, this.proxy);

    debug('[%s] broker server received a connection', req && req.headers ? req.headers['sec-websocket-key'] : 'unknown identifier');

    if (this.config.socket) {
      this.proxy.log.info('Connection established with a new backend');
    }
    else {
      this.proxy.log.info(`A connection has been established with backend ${req.connection.remoteAddress}.`);
    }
  ...
  addClientConnection(connection) {
    ...
    this.broadcastMessage('connection', new RequestContext({connection}));
  ...
  removeClientConnection(connection) {
    ...
    this.broadcastMessage('disconnect', new RequestContext({connection}));
    this.proxy.backendHandler.removeClient(connection.id);
