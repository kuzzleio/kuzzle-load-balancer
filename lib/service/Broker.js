...
Broker.prototype.resendPending = function() {
  ...
  Object.keys(pendingBatch).forEach(requestId => {
    ...
    this.brokerCallback(pendingBatch[requestId].message, pendingBatch[requestId].callback);
  });
};
...
Broker.prototype.brokerCallback = function (message, callback) {
  ...
  if (!message.data || !message.data.request) {
    callback(new Error(`Bad format : ${message}`));
  }
  else if (!message.data.request.requestId && message.room === 'request') {
    callback(new Error(`Bad message : ${message}`));
  }
  else if (!backend) {
    ...
    // TODO: Check if the remove is necessary, in any case should not
