 ...
 */
Broker.prototype.init = function (backendMode, context, webSocketOptions, backendTimeout) {
...
Broker.prototype.onConnection = function (backendSocket) {
  ...
  async.forEachOf(clientConnections, (clientConnection, requestId, callback) => {
    ...
    /** TODO: Manage error */
    /** TODO: Only add a backend if the http port has been provided first */
    this.context.backendConnectionStore.add(backend);

    if (this.context.backendConnectionStore.count() === 1) {
      this.resendPending();
    }
  });
};
...
Broker.prototype.brokerCallback = function (message, promise) {
  var
    backend = this.context.backendConnectionStore.getOneBackend(this.backendMode),
    pendingItem = {
      message,
      promise
    };
...
Broker.prototype.broadcastMessage = function (messageObject) {
  var
    backends = this.context.backendConnectionStore.getAllBackends(),
