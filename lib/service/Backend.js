  ...
  return {
    ...
    joinChannel: (data) => {
      ...
      if (client && client.protocol) {
        try {
          _proxy.protocolStore.get(client.protocol).joinChannel(data);
        }
        catch (e) {
          _proxy.log.error(`[Join Channel] Protocol ${client.protocol} failed: ${e.message}`);
        }
      }
    },
    leaveChannel: (data) => {
      ...
      if (client && client.protocol) {
        try {
          _proxy.protocolStore.get(client.protocol).leaveChannel(data);
        }
        catch (e) {
          _proxy.log.error(`[Leave Channel] Protocol ${client.type} failed: ${e.message}`);
        }
      }
    },
    notify: (data) => {
      ...
      if (client && client.protocol) {
        try {
          _proxy.protocolStore.get(client.protocol).notify(data);
        }
        catch (e) {
          _proxy.log.error(`[Notify] Protocol ${client.protocol} failed: ${e.message}\nNotification data:\n%s`, util.inspect(data, {depth: null}));
        }
      }
    },
    broadcast: (data) => {
      ...
      data = cleanStackFromError(data);
      Object.keys(_proxy.protocolStore.protocols).forEach(protocol => {
        debug('[%s] broadcasting through protocol "%s"', this.socketIp, protocol);

        try {
          _proxy.protocolStore.protocols[protocol].broadcast(data);
        }
        catch (e) {
          _proxy.log.error(`[Broadcast] Protocol ${protocol} failed: ${e.message}\nNotification data:\n%s`, util.inspect(data, {depth: null}));
        }
      });
    }
  };
...
Backend.prototype.onConnectionError = function(error) {
  debug('[%s] connection with backend errored: %a', this.socketIp, error);
...
Backend.prototype.sendRaw = function(room, data, callback) {
  debug('[%s] sending raw message to backend in room "%s": %a', this.socketIp, room, data);

  this.socket.send(JSON.stringify({room, data}), callback);
};

const cleanStackFromError = (data) => {
